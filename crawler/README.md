...
- 每日调用预算写入 `/app/data/.crawler_quota.json`，按北京时间日期重置，但不是 00:05 后立刻全部释放：默认 11:00 释放 50%，23:00 释放 100%，给人工使用小程序留出白天额度。

默认暂停和预算可以通过环境变量调整：

- `CRAWLER_RESET_GRACE_MINUTES=5`
- `CRAWLER_QUOTA_FIRST_RELEASE_HOUR=11`
- `CRAWLER_QUOTA_SECOND_RELEASE_HOUR=23`
...
- 额度释放：默认 11:00 前不主动爬；11:00-22:59 最多使用当天 50%；23:00 后可使用当天 100%。
...
- 每日北京时间 00:05 以后清除风控暂停，但默认到 11:00 才释放第一半爬虫预算。
