"""Administrator runtime status used by the admin routes."""
...
class AdminService:
    ...
    def dashboard(self, limit: int = 40) -> dict:
        """Return the total shown in the current admin dashboard.

        ``limit`` remains in the signature for the thin compatibility helper
        in ``app.http.server``; the removed user-summary feature no longer
        needs it.
        """
