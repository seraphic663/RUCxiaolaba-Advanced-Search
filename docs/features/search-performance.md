...
| 类型 | 无 Bigram 中位数 | Bigram 中位数 | 提升 |
|---|---:|---:|---:|
| 两字正文（食堂、选课） | 560-645ms | 83-90ms | 6.2-7.8x |
| 低命中两字正文（六一） | 923ms | 17ms | 54.3x |
| 两字全文含评论 | 5.8-6.1s | 118-133ms | 43.9-51.4x |
| 低命中两字全文（六一） | 5.7s | 26ms | 216.6x |
...
- 正文首屏：约 `1.3-14x` 加速。
- 正文+评论首屏：约 `3.3-71x` 加速。
- 所有测试排序的前 50 条结果与旧完整查询一致。
- 页面显示 `已找到 / 已检查 / 候选总数`，搜索完成前不显示虚假的准确总数。
- 上一页使用浏览器内页面缓存，保留正文展开、评论展开和已加载评论。
...
```powershell
python -m tools.benchmarks.benchmark_search_backends --repeats 3 --json-output temp\search_benchmark.json
```

结果：12 个案例的完整命中 ID 集合全部一致。

| 查询 | 范围 | 无 Bigram 中位数 | Bigram 中位数 | 加速 |
|---|---|---:|---:|---:|
| 食堂 | 正文 | 651 ms | 180 ms | 3.62x |
| 选课 | 正文 | 877 ms | 124 ms | 7.05x |
| 六一 | 正文 | 665 ms | 22 ms | 30.25x |
| 食堂 | 正文+评论 | 5294 ms | 119 ms | 44.58x |
| 选课 | 正文+评论 | 5482 ms | 101 ms | 54.48x |
| 六一 | 正文+评论 | 5153 ms | 12 ms | 435.96x |
| 图书馆 | 正文+评论 | 185 ms | 130 ms | 1.42x |
| 校园卡 | 正文+评论 | 74 ms | 65 ms | 1.14x |
| 猫 | 正文+评论 | 5382 ms | 5637 ms | 0.95x |

说明：

- 两字词是 Bigram 的主要收益区间，尤其是带评论的全文搜索。
- 三字及以上全文查询原本可使用 trigram，因此增益较小。
- 单字查询两种配置都回退 `LIKE`，速度差异属于缓存和运行波动。
- benchmark 会丢弃超过 `--max-sample-seconds` 的暂停/休眠污染样本。
