...
- API 返回候选总数、已扫描数、累计命中、下一游标和总数是否准确。
- 主页与 Admin 支持上一页/下一页游标、真实扫描进度和请求取消。
- 返回上一页时恢复页面缓存，保留正文和评论展开状态。
- 新增游标分页正确性测试和真实主库首屏性能 benchmark。
...
- 新增 Bigram 有/无对照 benchmark，分别检查完整结果正确性和中位数/P95 延迟。
