...
| DB 上限 | 3.8GB → 几乎没有余量 | 3.8GB → 还剩 ~30GB |
...
SQLite 不自己申请大内存，但依赖 Linux 内核的文件缓存。3.8GB 的 DB：
...
**DB 文件很大（3.8GB），不要用 scp 直接传，容易中断。** 推荐 rsync：
...
echo '{"count": 0}' > /app/data/checkin_count.json
echo "" > /app/data/feedback.jsonl
```
...
```cron
# 新帖：每 20 分钟
*/20 * * * * cd /app && python3 crawler_db.py new --db-path /app/data/posts.db --pages 500 --min-pages 20 --stop-unchanged 300 >> /var/log/crawler_new.log 2>&1

# 活跃帖刷新：每 40 分钟
3,43 * * * * cd /app && python3 crawler_db.py refresh --db-path /app/data/posts.db --pages 500 --min-pages 20 --stop-unchanged 300 >> /var/log/crawler_refresh.log 2>&1

# 历史补全：每天凌晨 3 点
5 3 * * * cd /app && python3 crawler_db.py backfill --endpoint lists --db-path /app/data/posts.db --start-page 200 --pages 500 --min-pages 20 --stop-unchanged 600 >> /var/log/crawler_backfill.log 2>&1
```
...
```bash
# crontab 加一条
10 2 * * * cd /app && python3 scripts/backup_runtime.py --data-dir /app/data --keep 30
```
...
# 每周备份
# crontab: 0 4 * * 0
python3 scripts/backup_runtime.py --data-dir /app/data --include-db --keep 2
...
3. **磁盘监控**：3.8GB DB 每天增长有限，但建议每月检查磁盘使用率 `df -h`。
4. **不要在线上 VACUUM**：VACUUM 需要额外 ~3.8GB 临时空间，可能撑满磁盘。如需回收空间，在本地做然后重新上传。
